...
public class DrawDAO {
	...
	public void insertLog(HashMap<String, String> log, int msgCode) {
		String sql = "INSERT INTO grabber_log ([game_code], [market], [draw_number], [result_time], [result], [message_code]) VALUES "
				+ "('" + log.get("gameCode") + "','" + log.get("market") + "','" + log.get("drawNumber") + "','"
				+ log.get("drawResultTime") + "','" + log.get("result") + "'," + msgCode + ")";
		genericHibernateDao.executeSql(sql);
	}
	...
	public void saveDrawMark6(Draw draw) {
		genericHibernateDao.save(draw);
	}
	
	public int selectDrawMarketMark6() {
		String sql = "SELECT last_draw from draw_market where game_code='MARK6' and market='HK'";
		return genericHibernateDao.scalarBySql(Integer.class, sql);
	}

	public List<Draw> selectLastMark6() {
		String sql = "SELECT TOP (1) * from draw where game_code='MARK6' and market='HK' order by draw_id DESC";
		List<Draw> drawlist = genericHibernateDao.findBySql(Draw.class, sql);
		return drawlist;
	}
	
	public void updateDrawMarketMark6(int lastDraw) {
		String sql = "UPDATE draw_market SET last_draw = '" + lastDraw + "' where game_code='MARK6' and market='HK'";
		genericHibernateDao.executeSql(sql);
	}
